...
| MSC firmware drive | ✅ `msc_firmware_disk` (geometry and published FAT, README file, non-image writes ignored, ascending-order rule, directory-entry commit, verification refuses a bad image, boot partition unchanged) | not implemented | not implemented | ✅ `examples/FirmwareMSC` |
