...
| HID keyboard raw report | ✅ `descriptor` | ✅ `hid_keyboard` | ✅ `hid_keyboard` normal port; **reverse port aborts with EspUsbHost 2.9.0** (`abort()` in the `usb.end()` → `usb.begin(HS)` span; deterministic; passes with 2.8.0 and the same device library; which assert fires is unconfirmed - the ELF that was symbolised had been rebuilt for 2.8.0 by then. Reported to EspUsbHost 2026-09-16, who found `end()` on P4 drops root-port power that only `experimentalForceFullSpeed` restores) | |
