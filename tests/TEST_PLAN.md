...
| USB DFU | ✅ `dfu_descriptor` (Download / Runtime descriptor bytes, zero endpoints, composite cost, function string, second-function rejection, OTA target) | ✅ `usb_dfu` (DFU interface beside a keyboard with no endpoints of its own, functional descriptor read off the wire, bad magic → `errWRITE`, failed verification → `errVERIFY`, CLRSTATUS recovery, ABORT, boot partition unchanged) | not implemented | ✅ `examples/FirmwareDFU` / `FirmwareHTTP` / `FirmwareBootMode` |
| MSC firmware drive | ✅ `msc_firmware_disk` (geometry and published FAT, README file, non-image writes ignored, ascending-order rule, directory-entry commit, verification refuses a bad image, boot partition unchanged; UF2: blocks written in reverse order, a duplicated block not counted twice, wrong family ID refused) | not implemented | not implemented | ✅ `examples/FirmwareMSC` |
| Single-cable boot mode (ESP32-S3) | ✅ `rom_dfu_guard` (`rebootToRomDfu()` returns `ESP_ERR_NOT_SUPPORTED` without restarting when `USB_PHY_SEL` is unburned) | not implemented | not implemented | ✅ `tests/manual/s3_single_cable` (the connector the sketch used comes back as the ROM loader; `rebootToRomDfu()` refuses instead of going dark) |
| Device interface GUID / vendor revision | ✅ `descriptor` (the GUID on the wire, a changed GUID moving the derived revision, an explicit revision winning, six malformed GUIDs refused at `begin()`) | not implemented | not implemented | ✅ `tests/manual/windows_device_guid` (Windows keeps the GUID it was sent, and keeps the *old* one when the revision is pinned - the control that makes it a test) |
...
| USB Video (UVC) | ✅ `video_descriptor` (MJPEG/YUY2 descriptor walk, composite numbering, FIFO budget, rejects) | not implemented | ✅ `manual/windows_uvc` (Windows 11: usbvideo bound, 181/181 frames byte-exact) | ✅ `examples/VideoCamera` |
...
Per-port binding requires `EspUsbHost` 2.8.0.
...
47. ✅ `single/video_descriptor` + `manual/windows_uvc` (UVC camera: descriptor walked without a host, then Windows 11 binding usbvideo and 181/181 frames byte-exact. Found on the way: a 1023-byte isochronous endpoint does not fit the S3's transmit FIFO and fails silently, so `begin()` now checks the FIFO budget)
48. ✅ `single/hid_registration_order` + `peer/composite_vendor_hid` (found → fixed: a single HID class registered after a non-HID class was looked up by TinyUSB instance number as a table position, so it returned no report descriptor and sent reports to a non-existent instance; the peer test registers Vendor before the keyboard and requires the host to receive the key, which enumeration alone did not catch)
