...
| MSC ファームウェアドライブ | ✅ `msc_firmware_disk`（幾何と公開する FAT、README ファイル、イメージでない書き込みの無視、昇順規則、directory entry による commit、不正イメージの検証拒否、boot partition が動かないこと） | 未実装 | 未実装 | ✅ `examples/FirmwareMSC` |
