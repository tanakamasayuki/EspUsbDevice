...
| # | 組み合わせ | 予測 | 根拠（仮説） |
|---|------------|------|--------------|
| 1 | HID + CDC | 動く見込み | EP1,2(HID) と EP3,4,5(CDC) が偶然 disjoint |
| 2 | HID + MIDI | 衝突疑い | HID EP1,2(カウンタ) を core アロケータが再配布 |
| 3 | HID + MSC | 衝突疑い | 同上 |
| 4 | HID + Vendor | 衝突疑い | 同上 |
| 5 | CDC + MIDI | 要確認 | CDC 固定 3,4,5 とアロケータ配布の競合 |
| 6 | CDC + MSC | 要確認（疑い） | 同上。EP 総数(3+1)も逼迫 |
| 7 | CDC + Vendor | 要確認 | 同上 |
| 8 | MIDI + MSC | 動く見込み | 両方同じアロケータ→重複回避 |
| 9 | MIDI + Vendor | 動く見込み | 同上 |
| 10 | MSC + Vendor | 動く見込み | 同上 |
| 11 | Audio + 任意 | NG（仕様） | コードで排他。**NG になることを確認** |
