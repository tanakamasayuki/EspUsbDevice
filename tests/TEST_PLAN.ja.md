...
| USB Audio | ✅ compile smoke | ✅ `usb_audio` speaker sink | ⚠️ `usb_audio` xfail（P4 は FS リンク + UAC2 vs UAC1 host） | | ✅ `examples/AudioSink` / `AudioSinkM5Speaker` |
...
`peer/usb_serial`、`peer/usb_midi`、`peer/usb_msc`、`peer/usb_vendor`、`peer/usb_audio` は
EspUsbDevice 実装へ移行済みです。Audio は speaker sink の peer test まで自動化済み。
`loopback/usb_audio` は追加済みだが P4 では `xfail`：1台 loopback のリンクは FS なのに、
デバイスはコンパイル時 `TUD_OPT_HIGH_SPEED` により UAC2 descriptor を出し、EspUsbHost は
UAC1 のみ解釈するため。解決は device 側の UAC1-at-FS 出し分け、または EspUsbHost への
UAC2 パーサ追加（`docs/DESIGN_NOTES.ja.md` 参照）。microphone path、長時間再生、実音確認も残作業。
...
32. ⚠️ `loopback/usb_audio`（P4 は xfail：FS リンク + UAC2 vs UAC1 host）
