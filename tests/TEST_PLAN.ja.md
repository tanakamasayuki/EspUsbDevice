...
EP 採番衝突（HID の endpoint が core のビットマスク未登録）は **案A で修正済み**：HID を EP1 duplex +
`reserve_endpoints=true` にした（`docs/DESIGN_NOTES.ja.md`「複合時の endpoint 採番衝突」）。修正後は全 class が
一貫採番されるため、**全ペアを個別に検証する必要はなく、予算天井まで積んだ最大構成テストが部分集合を包含する**
（4 クラスが `dup=0 / claimok=1` で機能すれば、その部分集合の各ペアも成立）。

| # | 組み合わせ | 結果 | 根拠 |
|---|------------|------|--------------|
| 1 | HID + CDC | ✅ 実機 OK（`composite_hid_cdc` 4/4） | EP1(HID) と EP3,4,5(CDC) が disjoint |
| 3 | HID + MSC | ✅ 修正後 OK（`composite_hid_msc`） | HID=EP1 予約、MSC=EP2（案A 修正の回帰） |
