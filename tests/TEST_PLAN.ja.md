...
| USB Audio | ✅ compile smoke | ✅ `usb_audio` speaker sink | 対象外（P4 の Audio は UAC2/HS、loopback は FS 限定） | | ✅ `examples/AudioSink` / `AudioSinkM5Speaker`（P4 HS） |
...
UAC2 自動化は可能（`docs/DESIGN_NOTES.ja.md` 参照）。`peer/usb_audio_mic` が USB Audio source
