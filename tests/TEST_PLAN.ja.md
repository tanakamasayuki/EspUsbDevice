...
| MSC ファームウェアドライブ | ✅ `msc_firmware_disk`（幾何と公開する FAT、README ファイル、イメージでない書き込みの無視、昇順規則、directory entry による commit、不正イメージの検証拒否、boot partition が動かないこと。UF2: 逆順書き込み、重複 block を二重に数えない、family ID 不一致の拒否） | 未実装 | 未実装 | ✅ `examples/FirmwareMSC` |
| 単一ケーブルの boot mode（ESP32-S3） | ✅ `rom_dfu_guard`（`USB_PHY_SEL` 未焼成時に `rebootToRomDfu()` が再起動せず `ESP_ERR_NOT_SUPPORTED`） | 未実装 | 未実装 | ✅ `tests/manual/s3_single_cable`（スケッチが使ったコネクタが ROM loader として戻る、`rebootToRomDfu()` が沈黙せず拒否する） |
| device interface GUID / vendor revision | ✅ `descriptor`（GUID が wire に載ること、GUID 変更で導出 revision が動くこと、明示指定が優先されること、不正な GUID 6 種を `begin()` が拒否すること） | 未実装 | 未実装 | ✅ `tests/manual/windows_device_guid`（Windows が送った GUID を保持し、revision を固定すると**古い方**を保持する。この対照があって初めてテストになる） |
...
| USB Video（UVC） | ✅ `video_descriptor`（MJPEG/YUY2 の descriptor 走査、複合時の採番、FIFO 予算、不正値の拒否） | 未実装 | ✅ `manual/windows_uvc`（Windows 11: usbvideo がバインド、181/181 フレームがバイト一致） | ✅ `examples/VideoCamera` |
...
Host 側は `EspUsbHostCdcSerial` をポートごとに 1 つずつ bind し（`setPort()`）、
**両ポートを個別に**確認します。

1. interface 4 本・endpoint 6 本が重複なく claim されること
2. device descriptor が IAD 由来の `0xef/0x02/0x01` を宣言してホストに届くこと
3. Host 側のポート番号と device 側が出した descriptor の対応（`getSerialPortInfo()` で
   port 0 = iface 0/1・bulk `0x82`/`0x02`、port 1 = iface 2/3・bulk `0x84`/`0x04`）。
   両者が「2 つ持っている」だけでなく**同じものを port 0 と呼んでいる**ことの確認
4. port 0 の双方向通信、port 1 の双方向通信
5. 分離: 受信カウントが port ごとに 1 ずつで、どちらの host 側 stream にも取り残しが無い
6. line coding が port 単位であること。port 1 だけ 57600 にして port 0 が 115200 のまま——
...
47. ✅ `single/video_descriptor` + `manual/windows_uvc`（UVC カメラ: ホスト無しで descriptor を走査し、そのうえで Windows 11 が usbvideo をバインドし 181/181 フレームがバイト一致。途中で判明: 1023 バイトの isochronous エンドポイントは S3 の送信 FIFO に入らず、しかも無言で失敗する。そのため `begin()` が FIFO 予算を検査するようにした）
48. ✅ `single/hid_registration_order` + `peer/composite_vendor_hid`（発見 → 修正: 非HIDクラスの後に登録した単独HIDクラスを TinyUSB のインスタンス番号でテーブル位置として引いていたため、report descriptor を返さず、report を存在しないインスタンスへ送っていた。peer テストは Vendor をキーボードより先に登録し、ホストがキーを受け取ることまで要求する。列挙だけでは捕まらなかった）
...
- `manual/` 配下のファイルを `test_*.py` という名前にしない。ここにあるものはすべて
  常時つながっているとは限らない実機か人間を必要とするので、収集経路から到達可能に
  してはならない。引数なしの `pytest` からも `pytest manual/` からも。

  担保するのは命名だけとする。`testpaths` も marker も意図的に使わない。同じ規約を
  2 通りで言うことになるうえ、名前を間違えたファイルが「実行されて見える」状態から
  「黙って飛ばされる」状態に変わってしまう。走ってはいけないものが走れば総数に出るが、
  走らないものは見えない。`tests/` に層を追加したとき宣言を待たず既定実行に入るのも
  同じ非対称性が理由である。
