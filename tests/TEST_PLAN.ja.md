...
| USBVendor / WebUSB | ✅ `descriptor` / compile | ✅ 列挙 `usb_vendor` | 予定 | | ✅ `examples/USBVendor` |
| USB Audio | | 予定 | | | |
...
`examples/USBVendor` の build-only 確認、`peer/usb_vendor` の interface / bulk endpoint 列挙から
開始します。bulk OUT -> Device -> bulk IN echo、control IN request `0x01`、control OUT request
`0x02` は、Host 側に非HID vendor bulk/control API が追加された後に自動化します。WebUSB BOS /
landing URL と Microsoft OS 2.0 descriptor は Host OS / browser / driver の影響が大きいため、
まず `tests/manual` の手順で確認します。
