...
| USB Audio | ✅ compile smoke | ✅ `usb_audio` speaker sink | 予定 | | ✅ `examples/AudioSink` / `AudioSinkM5Speaker` |
...
EspUsbDevice 実装へ移行済みです。Audio は speaker sink の peer test までを自動化済みで、
loopback、microphone path、長時間再生、実音確認は残作業です。
