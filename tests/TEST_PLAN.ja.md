...
| # | 組み合わせ | 結果 | 根拠 |
|---|------------|------|--------------|
| 1 | HID + CDC | ✅ 実機 OK（`composite_hid_cdc` 4/4） | EP1,2(HID) と EP3,4,5(CDC) が disjoint、CDC は core に予約済み |
| 2 | HID + MIDI | ✗ 非対応（同機構・予測） | HID の私的 EP が core マスク未登録 → 動的採番が衝突 |
| 3 | HID + MSC | ✗ **実機確定**（`begin()`==ESP_FAIL、`composite_hid_msc` xfail） | 同上。MSC が EP1 を再配布し HID EP1(OUT) と重複 |
| 4 | HID + Vendor | ✗ 非対応（同機構・予測） | 同上 |
| 5 | CDC + MIDI | ○ 予測 OK | CDC 固定 3,4,5 を予約、動的採番は残りから一貫採番 |
| 6 | CDC + MSC | ○ 予測 OK | 同上 |
| 7 | CDC + Vendor | ○ 予測 OK | 同上 |
| 8 | MIDI + MSC | ○ 予測 OK | 両方同じアロケータ→重複回避 |
| 9 | MIDI + Vendor | ○ 予測 OK | 同上 |
| 10 | MSC + Vendor | ○ 予測 OK | 同上 |
| 11 | Audio + 任意 | ✗ NG（仕様） | コードで排他。`unit/composite_reject` で固定 |

HID + {MSC, MIDI, Vendor} が動かない原因（HID の endpoint が core のビットマスクに登録されず、
動的採番クラスと衝突する）は `docs/DESIGN_NOTES.ja.md`「複合時の endpoint 採番衝突」に記録済み。
修正（HID を core 採番に載せる）は独立マイルストーン。それまで該当テストは `xfail`。
...
36. `peer/composite_hid_cdc`（複合の雛形 + 共通 util）
37. `peer/composite_hid_msc` / `hid_midi` / `hid_vendor`（破綻本命）
38. `peer/composite_cdc_*` / `midi_*` / `msc_vendor`（残りペア）
