...
確認します。この peer test はリリース前の Host 側 vendor bulk/control API を使うため、
Host リリースまでは `--profile=s3_peer_local` で確認します。Microsoft OS 2.0 descriptor は
Host OS / browser / driver の影響が大きいため、まず `tests/manual` の手順で確認します。
