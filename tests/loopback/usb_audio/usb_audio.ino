...
void setup()
{
  ...
  // P4 has a single high-speed UTMI PHY, so host and device cannot both run at
  // high speed on one board. The established loopback topology runs both roles at
  // full speed. The device-side audio descriptor is still generated by the
  // TUD_OPT_HIGH_SPEED (UAC2) path, so the host may report the stream format
  // fields as zero; the stream-based audioOutputStart() below selects the alt
  // setting directly and does not depend on those parsed format numbers.
