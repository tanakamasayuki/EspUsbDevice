...
## peer が「訊く」のに、ここが待っている理由

ここのテストはすべて `HOST_DEVICE` を待つところから始まります。スケッチ内のホスト側が
自分のデバイス側を列挙したときに 1 度だけ出す行です。`tests/peer` はまさにこの形をやめました。
列挙時に 1 度だけ出る行は最初に読んだテストにしか見えないので、peer のモジュールは
告知を待つのではなく訊く形（`?` → `DEVICE_READY <0|1>`）に変えています。

ここでは同じ変更が不要です。判断ではなく構造上の理由です。このディレクトリの各モジュールは
テスト 1 個で、専用のスケッチと専用の書き込みを持ちます。バナーを読むテストは、ボードが
起動した直後に必ず最初に走ります。それを読み損ねる 2 番目のテストが存在せず、依存する順序も
存在しません。

一度刺さったのは順序ではなくタイミングでした。スケッチが起動時にポート情報を出しており、
全体実行ではその出力が pytest のシリアル接続前に流れてしまうことがあり、スイート内では失敗し
単独では通る、という形で現れました。修正は、その報告を列挙後に移して `HOST_DEVICE` と一緒に
出すことでした。ここのモジュールに 2 つ目のテストが増えるか、列挙前に何かを報告し始めたら、
タイミングを調整し直すのではなく `peer/` の形に変換してください。
...
- `usb_serial_multi`: P4 1台上で CDC ACM を 3 ポート持つ device を high-speed controller
  ...
  port 1 / 2 へ書いたバイトが port 0 の経路に出てこないことを確認する。3 ポートすべてを host 側から個別に駆動し、
