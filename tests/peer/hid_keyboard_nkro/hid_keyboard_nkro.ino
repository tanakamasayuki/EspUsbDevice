...
//
// connectedAddress is latched in onDeviceConnected, which fires after the host has
...
static bool waitForDevice(uint32_t timeoutMs = 5000)
{
  ...
  while (connectedAddress == 0 && millis() - startedAt < timeoutMs)
  {
    delay(10);
  }
  return connectedAddress != 0;
}
