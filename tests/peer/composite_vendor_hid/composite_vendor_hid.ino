...
// Host side for the bulk Vendor + HID keyboard composite test - the same two
// functions as composite_hid_vendor, registered in the other order.
//
// The descriptors come out identical (HID is always emitted first), so this
// pairing exists for what the descriptors do not show: the library used to
// look the HID class up by TinyUSB's instance number as if it were a position
// in the registration table, which worked only while the HID class happened to
// be registered first. Registered after the Vendor class, its report descriptor
// request went unanswered and every report was sent to an instance that did not
// exist. The host receiving the key is the check; enumeration alone passed
// before the fix.

EspUsbHost usb;

static const uint16_t DEVICE_PID = 0x4026;
