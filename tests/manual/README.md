...
## `windows_uvc` (does a host accept the UVC camera and receive frames)

A UVC camera streaming MJPEG frames generated by `make_frames.py` (or a
procedural YUY2 pattern), plus the capture command and the byte-for-byte
comparison against what the device sent. It also prints the DWC2 transmit FIFO
registers, which is how the isochronous endpoint size was pinned down: a
1023-byte endpoint does not fit the S3's FIFO, is not refused, and streams
nothing.

Steps and the measured results: `windows_uvc/README.md`.
