...
**現在の既定値でのストリーミング**: 12 秒で 181 フレームを取得し、**その全部がデバイスの
送ったフレームとバイト単位で一致**、順序も連続で欠落も重複もなし。デバイス側は宣言 15 に対し
実測 15.16 fps、51 KB/s、失敗転送ゼロ、キャプチャ終了時にきれいに停止。

| | 値 |
|---|---|
| 取得フレーム数 | 181 |
| `frames.h` とバイト一致 | 181 / 181 |
| フレーム順序 | 連続、欠落なし |
| デバイス側レート | 15.16 fps、51 KB/s |
| 失敗転送 | 0 |
