...
- Sets a WebUSB landing URL with `EspUsbDeviceConfig::webusbEnabled` and
  `webusbUrl`.
...
- `UsbVendor.sendControlResponse(request, data, length)` responds to a control
  transfer.
- `config.webusbEnabled` enables this library's WebUSB BOS descriptor and, when
  this vendor function is present, its Microsoft OS 2.0 WinUSB descriptor.
- `config.webusbUrl` sets the WebUSB landing page URL. The URL descriptor scheme
  encodes `http://` and `https://` using the WebUSB URL scheme field.
...
- The Microsoft OS 2.0 descriptor targets the vendor interface number allocated
  by the composite descriptor builder. It is not advertised when no
  `EspUsbDeviceVendor` function is present.
- `EspUsbDevice` does not yet expose custom vendor-code, GUID, or Microsoft OS
  2.0 descriptor replacement APIs.
