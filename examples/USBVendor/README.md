...
- `UsbVendor.sendControlResponse(request, data, length)` responds to a control
  transfer.
- `config.webusbEnabled` enables Arduino-ESP32 TinyUSB core's WebUSB BOS
  descriptor.
- `config.webusbUrl` sets the WebUSB landing page URL. The URL descriptor scheme
  handling is owned by the core, so verify the expected host/browser format.
...
- WebUSB and Microsoft OS 2.0 descriptors are generated by the Arduino-ESP32
  TinyUSB core.
- `EspUsbDevice` currently exposes URL configuration and vendor request
  callbacks. It does not yet expose custom vendor-code or Microsoft OS 2.0
  descriptor replacement APIs.
