...
M5 ハードウェアで headset を実現する例です。M5 スピーカー再生（Host → device）と M5 マイク取り込み（device → Host）を同時に行います。

- 受信 PCM を PCMFlowDevice の `M5SpeakerBufferedPlayer` で `M5.Speaker` へ再生し、`M5.Mic` の取り込みを Host へ返します（両方向とも mono 16 kHz / 16-bit）。
- 全二重にはスピーカーとマイクが独立バスのハードウェア（例：CoreS3）が必要です。I2S 共有デバイス（M5StickC, Core2）では後から開始した方向のみ音が出ます。
