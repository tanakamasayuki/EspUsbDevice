...
- [x] EspBle へ NKRO API の変更依頼（`keys[]` → `bitmap[]` 改名ほか）。A〜E すべて 1.0.0 リリース前に対応済み。Host 側 `EspBleHidKeyboardState` にも同じ問題があり破壊的変更で改名され、EspUsbHost も 2.7.0 で追従。「bitmap を持つメンバは `bitmap`、usage 配列は `keys`」が姉妹 3 ライブラリ共通の規則になった（`EspBle/docs/DECISIONS.ja.md` 19）。`docs/ESPBLE_NKRO_API_CHANGE_REQUEST.ja.md`。
- [x] NKRO の状態送信 API。`sendReport(const EspUsbDeviceNkroKeyboardReport &)` で保持キー全体を 1 レポート送信、`heldState()` で最後に Host へ伝えた状態を公開。内部状態を公開型 1 個（`nkroState_`）に統一して private `setKeyBit()` を廃止し、bitmap レイアウトと modifier 振り分けの定義を 1 箇所に。`enableNkro()` 未実行時は失敗（boot protocol 時は従来どおり畳む）、重複送信の抑制は呼び出し側の責務。併せて `releaseUsage()` が modifier usage を落とさない穴と、増分 API が範囲外 usage を黙って無視する点も修正。EspBle と同一形状。host unit test `tests/unit/nkro_report` 追加。`docs/KEYBRIDGE_ADAPTER_PROPOSAL.ja.md` 項目① 参照。
- [ ] （follow-up）NKRO の peer テスト。7 キー以上を 1 report で送って Host 側が全キーを認識すること。sibling の EspUsbHost が NKRO bitmap をパースできるようになり次第（現状 NKRO のテストは 1 本も無い。`docs/DESIGN_NOTES.ja.md` の NKRO 節「未検証」と同じ依存）。
- [ ] （低優先）LED 状態の getter（`leds()`）追加。`onOutputReport()` は単一 slot で、統合レイヤが callback を占有するとスケッチ側から Lock 状態を読めない。LED は event でなく状態なので listener 基盤の新設は不要。`onProtocol()` / `onOutputReport()` の listener 化自体は保留（Device 側は EspBle も単一 slot で一貫しており、やるなら両者同時）。`docs/KEYBRIDGE_ADAPTER_PROPOSAL.ja.md` 項目② 参照。
