...
- [x] 複合時の endpoint 採番衝突の修正（案A・済）。HID を EP1 duplex + `reserve_endpoints=true` にして core のビットマスクに載せ、MSC/MIDI/Vendor の動的採番と衝突しないようにした。`docs/DESIGN_NOTES.ja.md`「複合時の endpoint 採番衝突」、`tests/peer/composite_hid_msc` 参照。
