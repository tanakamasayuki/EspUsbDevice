...
上記の通り 1台 loopback のリンクは FS だが、audio descriptor は**コンパイル時マクロ** `TUD_OPT_HIGH_SPEED`（P4 は 1）で
UAC2 として生成される（[USBAudioCard.cpp](../src/USBAudioCard.cpp)）。一方 `EspUsbHost` の audio パーサは UAC1(Audio 1.0) の
Type I format descriptor のみ対応のため、UAC2 の format を解釈できず `channels=0 / bits=0 / rate=0` となり、
`audioOutputStart()` が失敗する。このため **`loopback/usb_audio` は現状 fail** し、既知制約として扱う。

想定される解決策は2択:

1. Device 側で、宣言速度（`config_.speed`）に応じて UAC1/UAC2 を出し分ける（FS 宣言時は UAC1）。1台 loopback を通す最小の直し方。
2. `EspUsbHost` の audio パーサに UAC2（AS General の `bNrChannels` + Clock Source + UAC2 Type I）対応を追加する。Host 側で直す本命。

なお config descriptor は接続前に一度だけ組まれる（Arduino core の `tinyusb_load_enabled_interfaces()`）ため、
`tud_speed_get()` の実測速度で分岐する方式は取りづらく、選択根拠には「宣言速度」を使うのが現実的。
...
>
> よって capability（FS-only か HS 対応か）は**ハードで決まる前提**とし、EP サイズや UAC1/UAC2 などの
> descriptor は**ネゴした速度に追従して enumeration 時に出し分ける**方針にする（Step 2）。config からは
> port / speed の選択欄を無くした。詳細は「P4 USB ポート/PHY の実測整理」を参照。
...
Audio speaker sink は最小実装と peer test まで追加済みです。`loopback/usb_audio` も追加済みですが、
P4 では FS リンク上で UAC2 descriptor を出してしまい EspUsbHost(UAC1のみ)が解釈できないため現状 `xfail`
（上記「P4 USB ポート/PHY の実測整理」参照）。残作業は、P4 の UAC1/UAC2 速度対応、manual 確認、
