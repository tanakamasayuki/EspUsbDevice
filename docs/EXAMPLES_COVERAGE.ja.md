...
| `USBVendor` | `USBVendor` | 一部対応 | HID ではない vendor interface / bulk IN/OUT / control request。WebUSB URL と Microsoft OS 2.0 descriptor は未実装。 |
...
| Helper | 理由 | 優先度 |
|--------|------|--------|
| `EspUsbDeviceVendor` | bulk IN/OUT + control request は追加済み。WebUSB / Microsoft OS 2.0 descriptor が残項目。 | 中 |
...
2026-06-30 時点では 15 examples が通過しています。
