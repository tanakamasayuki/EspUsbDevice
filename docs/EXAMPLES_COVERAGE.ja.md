...
| `AudioCard` | `AudioSink` | 一部対応 | 最小 speaker sink。I2S bridge / codec / source は未対応。 |
...
`AudioSink` で単独 device の最小 speaker sink を追加済みです。複合 Audio device、I2S bridge、
codec 初期化、Audio source は別 example / 別マイルストーンで扱います。
...
| `EspUsbDeviceAudioSink` | 最小 sink は追加済み。source / I2S bridge / codec helper が残項目。 | 中 |
