...
| `USBVendor` | なし | 実装予定 | HID ではない vendor interface / WebUSB / control request。公式互換ではなく `EspUsbDeviceVendor` として仕様化する。 |
| `AudioCard` | なし | 未対応 | USB Audio class 未実装。 |
...
`EspUsbDevice` で対応する場合は、`EspUsbDeviceVendor` のような新 class が必要です。
HID report ではなく class/vendor interface descriptor と control transfer を扱うため、
仕様書先行で進めます。公式 Arduino-ESP32 `USBVendor` API との互換は目標にせず、`EspUsbDevice`
の使い方に合わせて単純な stream + callback API にします。
...
2. WebUSB:
   - BOS descriptor の WebUSB platform capability。
   - landing page URL descriptor。
   - vendor code による URL request。
   - browser から選択して echo / status を確認する example。
3. Windows 用 descriptor:
   - Microsoft OS 2.0 descriptor set を検討する。
   - WinUSB として扱う用途が明確になった時点で追加する。
...
公式 `AudioCard` は I2S bridge を含む本格 example です。
...
実装量が大きいため、まず Host 側既存テストと最小 Audio sink の仕様確認から始めます。
...
ただし firmware update は失敗時の復旧やサイズ制限が重要なため、現時点では example / docs 先行に留めます。
...
| Helper | 理由 | 優先度 |
|--------|------|--------|
| `EspUsbDeviceVendor` | 公式 `USBVendor` / WebUSB 相当。HID ではない vendor interface が必要。最小実装は bulk IN/OUT + control request。 | 中 |
| `EspUsbDeviceAudio` | AudioCard 相当。大きいが未対応 class として重要。 | 中 |
...
2026-06-29 時点では 14 examples が通過しています。
