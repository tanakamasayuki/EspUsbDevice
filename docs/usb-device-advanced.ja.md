...
8192 あたりで飽和し、4096/4096 はその 93% を、**FIFO だけ上げたときより少ない RAM** で出します。ホスト側の URB 深さを 2 より増やしても変わらないので、約 23 MB/s はホストではなく device 側の天井です。`CFG_TUD_HID_EP_BUFSIZE` と合わせて、P4 の既定は full speed 版より 7,168 バイト多く RAM を使います。TinyUSB がバッファを静的に確保するので、そのクラスを使わないスケッチでも消費します。
