...
片方向のストリームで効くのは送出ループではなく、送信FIFOの深さと転送長です。P4では同じ4 MiBの転送が、TinyUSB既定のFIFOと転送長で9.83 MB/s、このライブラリがP4に設定する4096/4096で21.5 MB/sになります（[5.4](#54-バッファのサイズ)）。データを作りながら流すスケッチは、`write()` が0を返すのをspinするのではなく `EspUsbDeviceVendor::waitWritable()` を使ってください。実測で帯域は同じ21 MB/sのまま、spinの回数が4 MiBあたり約25,000回から0になり、1転送につき1回だけblockする形になります。
